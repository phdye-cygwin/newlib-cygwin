...
int main( int argc, char * argv[] )
{
	...
	/* Use a mid-range priority that works without elevation.
	   Max priority (32) requires admin privileges on Windows/Cygwin.
	   Priority 15 maps to Windows THREAD_PRIORITY_ABOVE_NORMAL. */
	sp.sched_priority = param = 15;

	if ( sp.sched_priority < sched_get_priority_min( POLICY ) ||
	     sp.sched_priority > sched_get_priority_max( POLICY ) )
	{
		UNRESOLVED( errno, "Priority out of range" );
	}
	...
	/* Read back actual priority (OS may normalize the value) */
	ret = sched_getparam( 0, &sp );
	if ( ret != 0 )
	{
		UNRESOLVED( errno, "Failed to read back scheduling parameter" );
	}
	param = sp.sched_priority;  /* Use actual, not requested */
